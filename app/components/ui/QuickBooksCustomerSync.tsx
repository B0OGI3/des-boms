...
export function QuickBooksCustomerSync({
  onSyncComplete,
}: QuickBooksCustomerSyncProps) {
  ...
  return (
    <div>
        ...
        >
          <div>
            ...
            {result.deleted && result.deleted.length > 0 && (
              <Text size='xs' c='dimmed'>
                Removed {result.deleted.length} customers no longer in
                QuickBooks
              </Text>
            )}
          </div>
      ...
      {/* Help Text */}
      <Text size='xs' c='dimmed'>
        • <strong>Sync Customers:</strong> Full sync - imports new customers and
        removes deleted ones
        <br />• <strong>Import Only:</strong> Import customers from QuickBooks
        without removing any local data
      </Text>
    </div>
  );
}
