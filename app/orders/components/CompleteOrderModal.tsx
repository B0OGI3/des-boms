...
const submitOrderCompletion = async (
  order: Order,
  completedBy: string,
  ...
  >
    ...
    <Group gap='xl'>
      ...
      <div>
        ...
        <Badge color={getBadgeColor(order.priority)} variant='light' size='sm'>
          {order.priority}
        </Badge>
      </div>
      <div>
...
const StatusAlertSection: React.FC<{
  statusMessage: { type: 'success' | 'warning'; message: string } | null;
}> = ({ statusMessage }) => {
...
const CompletionRequirementsSection: React.FC<{ order: Order }> = ({
  order,
}) => (
  <div>
    ...
    <List size='sm' spacing='xs'>
      ...
      <List.Item
        ...
        style={{
          color:
            order.completedBatches === order.assignedBatches ? 'green' : 'red',
        }}
      >
...
const ModalHeader: React.FC<{ order: Order | null; canComplete: boolean }> = ({
  order,
  canComplete,
}) => (
  <Group gap='sm'>
    ...
    {order && (
      <Badge color={canComplete ? 'green' : 'orange'} variant='light'>
        {order.orderStatus}
      </Badge>
    )}
  </Group>
