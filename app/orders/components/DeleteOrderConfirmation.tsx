...
export const DeleteOrderConfirmation: React.FC<
  DeleteOrderConfirmationProps
> = ({ opened, onClose, order, onOrderDeleted }) => {
  ...
  const hasInProgressWork =
    order.status === 'IN_PROGRESS' || order.completedBatches > 0;
    ...
    >
      <Stack gap='lg'>
        ...
        >
          ...
          <Text size='sm' mt='xs'>
            Deleting this order will permanently remove it from the system along
        ...
        {hasInProgressWork && (
          ...
          >
            <Text fw={500} c='red'>
              Warning: Production Work Detected
            </Text>
            <Text size='sm' mt='xs'>
              This order has{' '}
              {order.status === 'IN_PROGRESS'
                ? 'work in progress'
                : 'completed batches'}
              . Deleting this order may affect production tracking and
              historical records.
            </Text>
          </Alert>
        )}
