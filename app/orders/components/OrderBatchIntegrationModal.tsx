...
export const OrderBatchIntegrationModal: React.FC<
  OrderBatchIntegrationModalProps
> = ({
  opened,
  onClose,
  order,
  ...
  const getOverallProgress = () => {
    ...
    const totalProgress = batches.reduce(
      (sum, batch) => sum + calculateBatchProgress(batch),
      0
    );
          ...
          <>
            ...
            >
              <Tabs.List>
                ...
                <Tabs.Tab
                  value='workstations'
                  leftSection={<IconTruck size='0.8rem' />}
                >
                  Active Workstations
                </Tabs.Tab>
              </Tabs.List>
                      ...
                      <Stack gap='xs'>
                        {getCurrentWorkstations().map(step => (
                          <Group key={step.id} justify='space-between'>
                            <Text size='sm'>
                              {step.workstation.name}: {step.description}
                            </Text>
                            <Button
                              size='xs'
                              variant='light'
                              onClick={() =>
                                onNavigateToWorkstation?.(step.workstation.id)
                              }
                            >
                              View
                            </Button>
