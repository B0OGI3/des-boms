...
export const SmartBatchGenerationModal: React.FC<
  SmartBatchGenerationModalProps
> = ({ opened, onClose, order, onBatchesCreated }) => {
  const [loading, setLoading] = useState(false);
  const [creating, setCreating] = useState(false);
  const [suggestions, setSuggestions] = useState<BatchGenerationData | null>(
    null
  );
      ...
      <Stack gap='lg'>
        ...
        <Card withBorder>
          <Group justify='space-between' align='center'>
            <div>
              <Text fw={600} size='md'>
                {suggestions?.orderNumber || order.orderId}
              </Text>
          ...
          <Card withBorder>
            ...
            <Group gap='xl'>
              <div>
                ...
                <Text fw={600} size='lg'>
                  {suggestions.summary.totalLineItems}
                </Text>
              </div>
              <div>
                <Text size='sm' c='dimmed'>
                ...
                <Text fw={600} size='lg'>
                  {suggestions.summary.totalBatches}
                </Text>
              </div>
              <div>
                <Text size='sm' c='dimmed'>
                ...
                <Text fw={600} size='lg'>
                  {suggestions.summary.estimatedCompletionDays} days
                </Text>
              </div>
              <div>
                <Text size='sm' c='dimmed'>
                  ...
                  {approvedSuggestions
                    .map(s =>
                      s.suggestedBatches.map(batch => {
                        ...
                        const totalMinutes = workflowSteps.reduce(
                          (sum, step) => sum + step.estimatedTime,
                          0
                        );
                        return (
                          <Badge
                            key={`${s.lineItemId}-${batch.batchNumber}`}
                            variant='light'
                            color='blue'
                            size='sm'
                          >
                            Batch {batch.batchNumber}: {workflowSteps.length}{' '}
                            steps ({totalMinutes}m)
                          </Badge>
                        );
                      })
                    )
            ...
            <Box mt='md'>
              <Group justify='space-between' align='center' mb='xs'>
                ...
                <Text size='sm' fw={500}>
                  {totalApprovedBatches} of {suggestions.summary.totalBatches}
                </Text>
              </Group>
              <Progress
                value={
                  (totalApprovedBatches / suggestions.summary.totalBatches) *
                  100
                }
                color='blue'
                size='sm'
        ...
        {suggestions && suggestions.suggestions.length === 0 && (
          <Alert color='blue' icon={<IconInfoCircle size={16} />}>
            All line items in this order already have complete batch
            assignments.
          </Alert>
        )}
          ...
          >
            <Stack gap='md'>
                    ...
                    {Math.round(
                      approvedSuggestions.reduce(
                        (sum, s) =>
                          sum +
                          s.suggestedBatches.reduce(
                            (bSum, b) => bSum + b.estimatedDuration,
                            0
                          ),
                        0
                      )
              ...
              <Text size='xs' c='dimmed' ta='center' mt='xs'>
                💡 All fields above are editable - click any value to customize
                before creating
              </Text>
            </Stack>
          </Card>
        ...
        <RoutingEditorModal
          ...
          currentSteps={(() => {
            ...
            const currentBatch = approvedSuggestions
              ...
              ?.suggestedBatches.find(
                b => b.batchNumber === routingEditorModal.batchNumber
              );
