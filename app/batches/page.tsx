...
import { BatchTableSkeleton, StatsSkeleton, FilterBarSkeleton } from "./components/LoadingSkeletons";
...
import { useSmartNotifications, BatchNotification } from "./components/SmartNotifications";
...
export default function BatchManagementPage() {
  ...
  useEffect(() => {
    if (mounted) {
      ...
      if (batchId) {
        ...
        setTimeout(() => {
          const targetBatch = batchSearch.batches.find(batch => batch.batchId === batchId);
          if (targetBatch) {
  ...
  const calculateBatchStats = (batches: Batch[]): BatchStats => {
    ...
    const rushBatches = batches.filter(batch => batch.priority === 'RUSH').length;
    ...
    const completedBatchesWithTimes = batches.filter(batch => 
      batch.status === 'COMPLETED' && 
      batch.startDate && 
    ...
    if (completedBatchesWithTimes.length > 0) {
      ...
      avgCompletionTime = Math.round(totalTime / completedBatchesWithTimes.length / (1000 * 60)); // Convert to minutes
    }
  ...
  const handleStatClick = (statType: 'all' | 'queued' | 'inProgress' | 'completed' | 'rush' | 'overdue') => {
  ...
  const isStatActive = (statType: 'all' | 'queued' | 'inProgress' | 'completed' | 'rush' | 'overdue'): boolean => {
    switch (statType) {
      case 'all':
        return batchSearch.statusFilter === 'ALL' && batchSearch.priorityFilter === 'ALL';
      case 'queued':
              ...
              <Group gap="md" align="center" style={{ marginBottom: 12 }}>
                ...
                <div>
                  ...
                  <Text size="md" className={styles.subtitle}>
                    Manufacturing Batches • Routing Steps • Work Travelers • Progress Tracking
                  </Text>
                </div>
                ...
                <Group gap="xs" style={{ marginLeft: 'auto' }}>
                  <Button
                    variant="subtle"
                    size="xs"
                    leftSection={<div style={{ color: '#3b82f6', fontSize: '10px' }}>🔔</div>}
            ...
            <Group gap="lg" align="flex-end">
              ...
              <div style={{ minWidth: "160px" }}>
                ...
                <select
                  ...
                  onChange={(e) => batchSearch.setStatusFilter(e.target.value as BatchStatusFilter)}
                  style={{
                    width: "100%",
                    height: "44px",
              ...
              {showAdvancedFilters && (
                <div style={{ minWidth: "140px" }}>
                  ...
                  <select
                    ...
                    onChange={(e) => batchSearch.setPriorityFilter(e.target.value as BatchPriorityFilter)}
                    style={{
                      width: "100%",
                      height: "44px",
                      background: "rgba(30, 41, 59, 0.6)",
              ...
              <Button
                ...
                style={{
                  ...
                  boxShadow: bulkSelectionMode ? "0 4px 12px rgba(139, 92, 246, 0.3)" : "none",
                  transition: "all 0.2s ease",
                }}
              >
