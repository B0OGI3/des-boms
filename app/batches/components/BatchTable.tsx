...
// Helper for routing step status color (used by BatchRow)
...
// Per-row BatchRow component so hooks can be used per row (avoids hook-in-loop)
interface BatchRowProps {
  batch: Batch;
  isExpanded: boolean;
  toggleRowExpansion: (batchId: string) => void;
  selectedBatches: Set<string>;
  onSelectBatch?: (batchId: string) => void;
  onViewBatch: (batch: Batch) => void;
  onEditBatch: (batch: Batch) => void;
  openRoutingEditor: (batch: Batch) => void;
  onStartBatch?: (batch: Batch) => void;
  onViewWorkTraveler?: (batch: Batch) => void;
  onDeleteBatch: (batch: Batch) => void;
  bulkSelectionMode: boolean;
  bomStructures: Map<string, any>;
}
...
const BatchRow: React.FC<BatchRowProps> = ({
  ...
  const handleOpenRouting = React.useCallback(
    () => openRoutingEditor(batch),
    [openRoutingEditor, batch]
  );
  ...
  const handleViewTraveler = React.useCallback(
    () => onViewWorkTraveler?.(batch),
    [onViewWorkTraveler, batch]
  );
          ...
          <div>
            <div className='flex items-center gap-2 mb-1'>
              ...
              <PartTypeIndicator
                ...
                hasComponents={
                  bomStructures.get(batch.lineItem.part.id)?.bomComponents
                    ?.length > 0
                }
                componentCount={
                  bomStructures.get(batch.lineItem.part.id)?.bomComponents
                ...
                bomLevels={
                  bomStructures.get(batch.lineItem.part.id)?.summary?.levels ||
                  0
                }
                size='xs'
                showDetails={true}
      ...
      {isExpanded && (
        <Table.Tr>
          <Table.Td
            ...
            style={{ background: 'rgba(241, 245, 249, 0.8)', padding: '16px' }}
          >
            <Stack gap='md'>
              {/* BOM Structure Section */}
              {batch.lineItem.part.partType !== 'RAW_MATERIAL' && (
                <div>
                  <Group align='center' gap='sm' style={{ marginBottom: 8 }}>
                    <IconComponents size={16} style={{ color: '#3b82f6' }} />
                    <Text fw={600} style={{ color: '#111827' }}>
                      BOM Structure
                    </Text>
                  </Group>
                  ...
                  {(() => {
                    const bomData = bomStructures.get(batch.lineItem.part.id);
                    ...
                    if (
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text
                            size='xs'
                            style={{ color: '#a16207', marginTop: 4 }}
                          >
                            Consider defining a BOM to track material
                      ...
                      >
                        ...
                        >
                          <Text size='sm' fw={500} style={{ color: '#0f172a' }}>
                            {bomData.summary.totalComponents} components across{' '}
                        ...
                        >
                            ...
                            .map((component: any) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    {component.quantity}{' '}
                                    {component.unitOfMeasure || 'EA'}
                                  </Badge>
                                </Group>

                                <Text
                                  size='xs'
                                  style={{ color: '#374151', lineHeight: 1.2 }}
                                >
                                ...
                                {component.componentCost > 0 && (
                                  <Text
                                    size='xs'
                                    style={{ color: '#059669', marginTop: 2 }}
                                  >
                          ...
                          {bomData.bomComponents.length > 6 && (
                            ...
                            >
                              <Text size='xs' style={{ color: '#6b7280' }}>
                                +{bomData.bomComponents.length - 6} more
                                components
                              </Text>
                            </div>
                          )}
                      ...
                      .map(step => (
                        <div
                          ...
                          style={{
                            ...
                            borderRadius: '8px',
                            borderWidth:
                              step.id === currentStep?.id ? '2px' : '1px',
                            borderStyle: 'solid',
                            borderColor:
...
export function BatchTable({
  ...
  // Top-level helper for step status color
          ...
          <Table.Tbody>
            {batches.map(batch => (
              <BatchRow
                key={batch.id}
                batch={batch}
                isExpanded={expandedRows.has(batch.id)}
                ...
                onViewWorkTraveler={onViewWorkTraveler}
