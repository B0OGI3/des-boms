...
export default function PartsDemo() {
  ...
  const [bomStructure, setBomStructure] = useState<BOMStructure | null>(null);
  const [loading, setLoading] = useState(false);
  const [filter, setFilter] = useState<'ALL' | 'FINISHED' | 'SEMI_FINISHED' | 'RAW_MATERIAL'>('ALL');
  ...
  const loadBOMStructure = async (part: Part) => {
    setLoading(true);
    setSelectedPart(part);
    try {
      const response = await fetch(`/api/bom?partId=${part.id}&levels=2`);
      const result = await response.json();
      if (result.success) {
        setBomStructure(result.data);
      }
    } catch (error) {
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6">
        <div className="flex gap-2">
          {['ALL', 'FINISHED', 'SEMI_FINISHED', 'RAW_MATERIAL'].map((filterOption) => (
            <button
              key={filterOption}
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {parts.map((part) => (
                <button
                  key={part.id}
                  type="button"
                  onClick={() => loadBOMStructure(part)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      loadBOMStructure(part);
                    }
                  }}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs ${getPartTypeColor(part.partType)}`}>
          ...
          {selectedPart ? (
            <div>
              ...
              {bomStructure?.summary ? (
                ...
                <p className="text-gray-600">Loading BOM structure...</p>
              )}
            </div>
          ) : (
            <p className="text-gray-600">Select a part to view its BOM structure</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-8 bg-gray-50 p-6 rounded-lg">
        <h3 className="text-lg font-semibold mb-4">Parts Master System Features</h3>
