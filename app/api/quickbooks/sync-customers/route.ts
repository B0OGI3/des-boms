...
async function deleteLocalCustomersNotInQB(
  qbIds: Set<string>
): Promise<string[]> {
  const localCustomers = await prisma.customer.findMany();
  const toDelete = localCustomers.filter(
    c => c.quickbooksId && !qbIds.has(c.quickbooksId)
  );
...
async function initializeQuickBooksService(): Promise<
  { service: any } | { error: NextResponse }
> {
  try {
    ...
    return {
      error: NextResponse.json(
        {
          error: 'Failed to get QuickBooks service',
...
async function fetchQuickBooksCustomers(
  qbService: any
): Promise<{ customers: QBCustomer[] } | { error: NextResponse }> {
  try {
...
async function importCustomers(
...
): Promise<{ imported: number } | { error: NextResponse }> {
  let imported = 0;
  for (const qb of qbCustomers) {
    try {
      ...
      console.error(
        'Prisma upsert error for customer',
        qb.PrimaryEmailAddr?.Address,
        err
      );
      return {
        error: NextResponse.json(
          {
            ...
            details: err instanceof Error ? err.message : String(err),
            customer: qb.PrimaryEmailAddr?.Address,
          },
          { status: 500 }
        ),
      };
...
async function cleanupLocalCustomers(
  qbIds: Set<string>
): Promise<{ deleted: string[] } | { error: NextResponse }> {
  try {
