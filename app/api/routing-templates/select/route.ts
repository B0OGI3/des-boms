  ...
  try {
    ...
    if (!quantity || !priority) {
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required fields: quantity, priority',
        },
        { status: 400 }
      );
    }
...
function selectOptimalTemplate(params: any) {
  ...
  if (
    partName.includes('assembly') ||
    partName.includes('assy') ||
    partName.includes('kit')
  ) {
    return {
  ...
  if (
    precisionMaterials.some(mat => materialSpec.includes(mat)) ||
    precisionKeywords.some(keyword => partName.includes(keyword)) ||
    specialRequirements.some((req: string) =>
      precisionKeywords.some(keyword => req.includes(keyword))
    )
  ) {
    return {
  ...
  if (
    prototypeKeywords.some(keyword => partName.includes(keyword)) ||
  ...
  const templates: any = {
    ...
    'template-precision': {
      ...
      steps: [
        ...
        {
          stepNumber: 5,
          workstationId: 'ws-qa-1',
          workstationName: 'QA-1',
      ...
      steps: [
        {
          ...
          workstationId: 'ws-mill-2',
          workstationName: 'MILL-2',
          description: 'Prototype machining',
          estimatedTime: 120,
          required: true,
        },
        {
          ...
          workstationId: 'ws-inspect-1',
          workstationName: 'INSPECT-1',
          description: 'Development inspection',
          estimatedTime: 30,
          required: true,
        },
        {
          stepNumber: 3,
          workstationId: 'ws-engineering',
          workstationName: 'ENGINEERING',
