...
function QuickBooksSuccessPageInner() {
...
QB_COMPANY_ID="${companyId}"` +
  ...
  const handleCopyCommand = () => {
    const command =
      `node update-qb-tokens.js "${accessToken}" "${companyId}"` +
    ...
    <Container size='md' style={{ marginTop: 40 }}>
      <Stack gap='lg'>
        ...
        <Alert color='blue' title='Next Steps - Add Tokens to Environment'>
          <Stack gap='md'>
            <Text>
              Copy the following values and add them to your <Code>.env</Code>{' '}
            ...
            <Alert color='orange' title='Quick Update Option'>
              <Text size='sm'>
                You can also use our helper script to automatically update your
                .env.local file:
              </Text>
              <Code
                block
                style={{ padding: '8px', fontSize: '12px', marginTop: '8px' }}
              >
                node update-qb-tokens.js &quot;{accessToken}&quot; &quot;
                {companyId}&quot;{refreshToken && ` "${refreshToken}"`}
              </Code>
            </Alert>
            {refreshToken && (
        ...
        <Alert color='green' title='What happens next?'>
          <Stack gap='xs'>
            <Text>
              • Customer data will now sync automatically between DES-BOMS and
              QuickBooks
            </Text>
            <Text>
              • New customers created in your orders will appear in QuickBooks
            </Text>
            <Text>
              • Changes made in QuickBooks will be reflected in your system
            </Text>
            <Text>
              • You can view sync status in your customer management interface
            </Text>
          </Stack>
        </Alert>
