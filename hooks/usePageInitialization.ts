...
export function usePageInitialization(
  ...
  const [pageInitialization, setPageInitialization] =
    useState<PageInitialization>(() => {
      return initialTasks.reduce(
        (acc, task) => ({ ...acc, [task]: false }),
        {}
      );
    });
  ...
  const initializeTask = async (
    ...
    try {
      ...
      return delayedStateUpdate(() =>
        updateInitializationState({ [taskName]: true })
      );
    } catch (error) {
  ...
  const initializePage = async (
    tasks: Array<{ name: string; fn?: () => Promise<void> | void }> = []
  ) => {
    setMounted(true);
    ...
    if (tasks.length === 0) {
      ...
      await Promise.allSettled(
        defaultTasks.map(task => initializeTask(task.name))
      );
    } else {
      await Promise.allSettled(
        tasks.map(task => initializeTask(task.name, task.fn))
      );
    }
