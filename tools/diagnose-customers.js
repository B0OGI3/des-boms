...
async function diagnoseCustomerSync() {
  ...
  try {
    ...
    console.log(
      `   Access Token: ${process.env.QB_ACCESS_TOKEN ? '✅ Present' : '❌ Missing'}`
    );
    console.log(`   Company ID: ${process.env.QB_COMPANY_ID || '❌ Missing'}`);
    console.log(
      `   Refresh Token: ${process.env.QB_REFRESH_TOKEN ? '✅ Present' : '❌ Missing'}`
    );
    ...
    if (localCustomers.length === 0) {
      ...
      console.log(
        '   2. In QuickBooks Integration section, click "Sync Customers"'
      );
      console.log('   3. Or go to New Order modal and it will auto-sync');
    } else {
      const qbCustomers = localCustomers.filter(c => c.quickbooksId);
      console.log(
        `   Found ${qbCustomers.length} QuickBooks customers out of ${localCustomers.length} total`
      );
